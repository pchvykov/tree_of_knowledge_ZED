...
ToK = function (svg, db) {
  ...
  var outer = svg.append("svg:svg").attr("pointer-events", "all");
  ...
  var initScale = 0.5;
  var initTransl = [
    (width / initScale - treeDim[0]) / 2,
    (height / initScale - treeDim[1]) / 2,
  ];
  var vis = outer
    .append("svg:g")
    .attr(
      "transform",
      "scale(" + initScale + ")" + " translate(" + initTransl + ")",
    )
    .call(zm)
  ...
  this.gravStrength = [
    visWindow[2] - visWindow[0],
    visWindow[3] - visWindow[1],
  ];
  ...
  defs.append("circle").attr("id", "circleNode").attr("r", 1);
  // Arrowhead triangle
  defs
    .append("polygon")
    .attr("id", "andNode")
  ...
  defs
    ...
    .attr(
      "points",
      "0.0,-1.2 0.3527,-0.4854 1.1413,-0.3708 0.5706,0.1854 0.7053,0.9708 0.0,0.6 -0.7053,0.9708 -0.5706,0.1854 -1.1413,-0.3708 -0.3527,-0.4854",
    );
  ...
  d3.select(window).on("keydown", gui.keydown);

  //==================== REDRAW =============================================================
  // pull data from server and redraw force layout
  var visSubscr, phantSubscr;
  this.redraw = function (postScript) {
    //execte postScrip() at the end
    ...
    db.subscribe(visWindow, function () {
      //parseInt($('#nnodesInput').val())
      ...
      nodeData.forEach(function (nd) {
        if (nd.x == 2345 || nd.y == 2345)
          console.error("unpositioned node: ", nd);
        ...
        nd.permFixed =
          nd.phantom || nd.permFixed || (nd.zoomLvl != minZoom && nd.x != 2346);
        ...
        nd.parentsIx = linkData //used to orient "derivation" triangles
          ...
          .map((lk) => nodeData.findIndex((ndDat) => ndDat._id == lk.source));
        nd.childrenIx = linkData
          .filter((lk) => nd._id == lk.source && lk.oriented)
          .map((lk) => nodeData.findIndex((ndDat) => ndDat._id == lk.target));
        nd.chiMinLen = ["id", Infinity];
        nd.parMinLen = ["id", Infinity]; //for percolating springs model
      });

      //replace node id-s in links with pointers to nodes
      //note: link.source prop used to distinguish links and nodes
      linkData.reduceRight(function (tmp, lk, idx, arr) {
        //a hack to get forEach runningh from the end (in reverse)
        lk.source = nodeData.find(function (nd) {
          return nd._id == lk.source;
        });
        lk.target = nodeData.find(function (nd) {
          return nd._id == lk.target;
        });
        if (!lk.source || !lk.target) arr.splice(idx, 1); //remove orphaned links
        // console.error("orphaned link! ", lk._id);}
        var strKey,
          ttZm = Session.get("currZmLvl");
        // do{
      ...
      //choose node shape:
      newNodes
        .append("use") //append("circle").attr("r",1)
      ...
      newNodes.each(function (d, idx) {
        ...
        var newTT = d3
          ...
          .attr("class", "tooltipO");
        newTT
          .datum(this) //store node DOM el't in datum
          .append("xhtml:span")
          .attr("class", "inner");
        this.tooltip = newTT; //newTT - d3 elt, this - DOM elt
      });

      d3.selectAll(".tooltipO .inner").text(function (d) {
        return d3.select(d).datum().title;
      });

      node.exit().each(function () {
        if (this.tooltip) this.tooltip.remove();
      });
      node.exit().select(".node").transition().attr("transform", "scale(0)");
      // .attr("r", 0);
      node.exit().remove();
      ...
      node
        ...
        .attr(
          "transform",
          (d) => "scale(" + d.importance * $("#sizeInput").val() + ")",
        )
      ...
      force.charge(function (d) {
        return (-$("#ChargeInput").val() / 2) * Math.pow(d.importance, 2);
      }); //*((d.phantom)?3:1) *((d.phantom)?0:1)
      ...
      link.each(function (d) {
        ...
        switch (d.type) {
          ...
          case "specialCase":
          case "used":
            $(this).css("stroke-width", w * 0);
            break;
          case "supports":
            // remove the stroke but keep the markers
            $(this).css(
              "stroke-dasharray",
              w * 2 + "," + w * (marker_spacing - 2),
            );
            break;
          ...
          case "implies":
            ...
            break;
          case "conjecture":
            $(this).css("stroke-dasharray", w * 4 + "," + w * 2);
            break;
          case "related":
            $(this).css("stroke-dasharray", w + "," + w * 2);
            break;
          default:
            console.log("unrecognized link type:", d.type, d);
        }
      });
      ...
      linkData.forEach(function (lk) {
        ...
        lk.minDist =
          (parseFloat(lk.source.importance) +
            parseFloat(lk.target.importance)) *
          2;
      ...
      if (postScript) {
        postScript();
        console.log("redraw passed postScript function");
  ...
  var runBt = svg
    .append("svg:g")
    .attr("id", "runButton")
    .attr("transform", "translate(" + (width - 50) + "," + 2.5 + ")")
    .on(
      "mousedown",
      () => {
        forceRun = true;
  ...
  runBt.append("rect").attr("width", 40).attr("height", 30);

  runBt.append("svg:text").text(">>>").attr("x", 3).attr("y", "1em");
  ...
  function positionTooltips() {
    d3.selectAll(".tooltipO").each(function (d, idx) {
      ...
      this.style.left =
        (bbox.left + bbox.right - this.firstChild.offsetWidth) / 2 -
        8 +
        window.scrollX +
        "px";
      this.style.top = bbox.top + window.scrollY + "px";
  ...
  this.updateSelection = function () {
    //update the CSS classes appropriately
    if (gui.selected) {
      link.classed("link_selected", function (d) {
        return d._id == gui.selected._id;
      });
      node.select(".node").classed("node_selected", function (d) {
        return d._id == gui.selected._id;
      });
    } else {
    ...
    if (gui.editPopup) {
      ...
      // console.log(editID)
      link.classed("link_edited", function (d) {
        return d._id == editID;
      });
      node.select(".node").classed("node_edited", function (d) {
        return d._id == editID;
      });
    } else {
  ...
  $("#calcZoom").click(function () {
    //recalculate the effective connectivit matrices on the server
    Meteor.call("calcEffConn", Session.get("currGraph"), function (err, res) {
      console.log(res);
      // tree.redraw();
    });
  });
  $("#calcGrWeights").click(function () {
    //recalculate the effective connectivit matrices on the server
  ...
  function tick(e) {
    ...
    linkData.forEach(function (lk) {
      ...
      var len = math.norm([delx, dely]) + lk.strength / 4,
        scale; //ensure denomenators >0
      //////////////////////////////////////////////////////////
      ////non-linear springs:---
      ...
      lk.strong = false;
      if (len < lk.target.parMinLen[1] || lk.target.parMinLen[0] == lk._id) {
        //Short spring
        lk.target.parMinLen[0] = lk._id;
        lk.target.parMinLen[1] = len * $("#linkDistMult").val();
        lk.strong = true;
      }
      if (len < lk.source.chiMinLen[1] || lk.source.chiMinLen[0] == lk._id) {
        lk.source.chiMinLen[0] = lk._id;
        lk.source.chiMinLen[1] = len * $("#linkDistMult").val();
        lk.strong = true;
      }
      if (lk.strong) {
        scale =
          (g / 50) *
          Math.pow(lk.strength, 2) *
          $("#linkSStrInput").val() *
          (1 - lk.minDist / len);
      } else {
        //Long spring
        scale =
          (g / 50) *
          Math.pow(lk.strength, 2) *
          ((lk.strength * $("#linkStrInput").val()) / len) *
          (1 - lk.minDist / len);
      }
      ////Linear springs:---------
      ...
      var dx = delx * scale,
        dy = dely * scale;
      if (lk.source.type == "derivation") {
        //make derivation node close to target node
        var nnScale = (0.5 * (len - lk.minDist)) / lk.strength;
        dx *= nnScale;
        dy *= nnScale; //non-linear springs
      }
      ...
      if (lk.oriented) {
        ...
        //   ));
        scale =
          (($("#linkOrtInput").val() * g * Math.pow(lk.strength, 3)) / len) *
        ...
        dx -= dely * scale;
        dy += delx * scale;
      } else if (lk.type == "theorem") {
        //orient orthogonal to flow
        scale =
          ((-$("#linkOrtInput").val() * g * Math.pow(lk.strength, 3)) / len) *
          Math.pow(delx / len, 2) *
          Math.sign(dely) *
      ...
      if (!lk.source.fixed) {
        lk.source.x += dx / srcChrg;
        lk.source.y += dy / srcChrg;
      } //divide by charge=mass to get acceleration
      if (!lk.target.fixed) {
        lk.target.x -= dx / trgChrg;
        lk.target.y -= dy / trgChrg;
      }
    ...
    nodeData.forEach(function (nd, idx) {
      ...
      if (!nd.fixed) {
        ...
        var dxG = nd.x - tree.gravTo[0];
        dyG = nd.y - tree.gravTo[1];
        nd.x -=
          (grav * e.alpha * Math.pow(dxG, 2) * Math.sign(dxG)) /
          tree.gravStrength[0]; //treeDim[0]/2);
        nd.y -=
          (grav * e.alpha * Math.pow(dyG, 2) * Math.sign(dyG)) /
          tree.gravStrength[1]; //treeDim[1]/2);
    ...
    link.attr("points", function (d) {
      if (d.type == "used" || d.type == "specialCase") {
        ...
        if (d.type == "used") {
          // triangle from source into target
          base = d.source;
          tip = d.target;
        } else {
          // triangle from target into source
          base = d.target;
          tip = d.source;
        }
    ...
    node.attr("transform", function (d) {
      ...
      if (
        d.type == "derivation" &&
        d.parentsIx.length > 0 &&
        d.childrenIx.length > 0
      ) {
        var rot =
          (180 / Math.PI) *
          Math.atan2(
            d.parentsIx.reduce((prev, idx) => prev + nodeData[idx].x, 0) /
              d.parentsIx.length - //average x of parents
              d.childrenIx.reduce((prev, idx) => prev + nodeData[idx].x, 0) /
                d.childrenIx.length, //average x of children
            -(
              d.parentsIx.reduce((prev, idx) => prev + nodeData[idx].y, 0) /
                d.parentsIx.length -
              d.childrenIx.reduce((prev, idx) => prev + nodeData[idx].y, 0) /
                d.childrenIx.length
            ),
          ); //y between parents and children c.o.m.
